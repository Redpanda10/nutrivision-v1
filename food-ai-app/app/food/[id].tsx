...
const COLORS = {
  ...
  danger: "#dc2626",
  white: "#ffffff",
  overlay: "rgba(255, 255, 255, 0.85)", // Muted white overlay
};
          ...
          <SafeAreaView style={styles.safeArea}>
            ...
            <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
              ...
              <View style={styles.card}>
                ...
                {/* MEAL BREAKDOWN LIST */}
                {entry.selectedFoods && entry.selectedFoods.length > 0 && (
                  <Section title="Meal Breakdown" icon="list">
                    {entry.selectedFoods.map((item: any, index: number) => (
                      <View key={index} style={styles.foodItemRow}>
                        ...
                        <View style={styles.foodItemCalories}>
                          <Text style={styles.itemCalsText}>
                            {Math.round((item.nutrition?.caloriesKcal || 0) * (item.weight / 100))}
                          </Text>
                          <Text style={styles.itemCalsLabel}> kcal</Text>
                        </View>
                      </View>
                    ))}
                  </Section>
...
const styles = StyleSheet.create({
  safeArea: { flex: 1 },
  darkOverlay: { flex: 1, backgroundColor: 'rgba(255, 255, 255, 0.4)' },
  ...
  statBox: { backgroundColor: COLORS.white, padding: 12, borderRadius: 18, alignItems: 'center', width: '23%', elevation: 2 },
  statValue: { fontSize: 18, fontWeight: '900', color: COLORS.text },
  statLabel: { fontSize: 9, color: COLORS.muted, fontWeight: '600', textTransform: 'uppercase' },
  statUnderline: { height: 3, width: 15, backgroundColor: COLORS.primary, borderRadius: 2, marginVertical: 4 },
  statName: { fontSize: 10, color: COLORS.text, fontWeight: '700' },
  ...
  foodItemInfo: { flex: 1 },
  foodItemName: { fontSize: 15, fontWeight: '700', color: COLORS.text, textTransform: 'capitalize' },
  foodItemWeight: { fontSize: 12, color: COLORS.muted },
  foodItemCalories: { flexDirection: 'row', alignItems: 'baseline' },
  itemCalsText: { fontSize: 14, fontWeight: '800', color: COLORS.primary },
  itemCalsLabel: { fontSize: 10, color: COLORS.muted },
  bulletRow: { flexDirection: 'row', alignItems: 'flex-start', marginBottom: 8, backgroundColor: COLORS.primaryLight, padding: 10, borderRadius: 12 },
  bulletText: { fontSize: 14, color: COLORS.primaryDark, marginLeft: 8, flex: 1, fontWeight: '600' },
  warningText: { color: COLORS.danger, fontSize: 14, marginBottom: 4, fontWeight: '600' },
  safeContainer: { flexDirection: 'row', alignItems: 'center', padding: 12 },
  safeNote: { color: COLORS.primary, fontWeight: '700', marginLeft: 8, fontSize: 13 },
});